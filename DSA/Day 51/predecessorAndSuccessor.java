...
class GfG {
    ...
    public static void findPreSuc(Node root, int key) {
         ...
         * code.
         */
        diff_pre = Integer.MAX_VALUE;
        diff_suc = Integer.MAX_VALUE;
        solve(root, key);
        return;
    }

    static int diff_pre, diff_suc;
    ...
    public static void solve(Node root, int key) {
        ...
        solve(root.left, key);
        if (root.data < key) {
            if (Math.abs(root.data - key) < diff_pre) {
                pre = root;
                diff_pre = Math.abs(root.data - key);
            }
        }
        if (root.data > key) {
            if (Math.abs(root.data - key) < diff_suc) {
                suc = root;
                diff_suc = Math.abs(root.data - key);
            }
        }
